document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnRegistrarDenuncia = document.getElementById('btn-registrar-denuncia');
    ...
    // CAMBIO IMPORTANTE: Referencia al SELECT REAL (OCULTO) para el tipo de documento
    // y las nuevas referencias para el Custom Select visible
    ...
    // --- Funciones para manejar Ubigeo desde el JSON local ---

    // Función para cargar el archivo JSON de Ubigeo
    ...
    // --- Lógica para el Custom Select de Tipo de Documento y control de Fecha de Emisión ---
    if (customSelectWrapper && customSelectTrigger && selectedValueSpan && customOptionsList && hiddenTipoDocumentoSelect) {
        // Inicializar la visibilidad de fechaEmisionGroup al cargar la página
        // basado en el valor inicial del select real oculto
    ...
    // --- Event Listeners para navegación de secciones ---

    // Listener para el botón "Continuar" de la primera sección
    ...
    if (btnRegistrarDenuncia) {
        btnRegistrarDenuncia.addEventListener('click', async (event) => {
            ...
            formData.append('fechaNacimiento', fechaNacimientoInput.value.trim());
            formData.append('fechaEmision', fechaEmisionInput.value.trim()); // Puede ser vacío, el backend lo maneja
            ...
            formData.append('fechaIncidente', fechaIncidenteInput.value.trim());
